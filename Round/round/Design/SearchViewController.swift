...
class SearchViewController: BaseViewController<FilterViewModel>, NavigationDesignProtocol {
    var navTitle: String
    ...
    override init(viewModel: FilterViewModel) {
        navTitle = viewModel.navigationTitle
        navDescription = ""
        navIcon = Icons.filter.image()
...
class UserCell : UITableViewCell, SearchableCell {
    ...
    func setup(_ model: SearchableModelProtocol) {
        guard let model = model as? UserModel else { return }
